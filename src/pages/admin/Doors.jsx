...
const emptyForm = () => ({ name: '', description: '', material: '', price_from: '', image_url: '' })

export default function Doors({ role }) {
  ...
  const fetchItems = async () => {
    setLoading(true)
    const { data } = await supabase.from('doors').select('*').order('created_at', { ascending: false })
    setItems(data || [])
    setLoading(false)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    const payload = { name: form.name.trim(), description: form.description.trim(), material: form.material.trim(), price_from: Number(form.price_from) || null, image_url: form.image_url, available: true }
    let error
    if (editing) {
      ({ error } = await supabase.from('doors').update(payload).eq('id', editing))
    } else {
      ({ error } = await supabase.from('doors').insert(payload))
    }
  ...
  const handleEdit = (item) => {
    setForm({ name: item.name, description: item.description || '', material: item.material || '', price_from: item.price_from || '', image_url: item.image_url || '' })
    setEditing(item.id); setShowForm(true)
    window.scrollTo({ top: 0, behavior: 'smooth' })
  }

  const handleDelete = async (id) => {
    if (!confirm('Delete this door design?')) return
    const { error } = await supabase.from('doors').update({ available: false }).eq('id', id)
        ...
        <div className="p-6 space-y-5 pb-24 md:pb-6">
          ...
          {showForm && (
            <form onSubmit={handleSubmit} className="card border-[#D4A04A]/15 p-5 fade-in">
              <h3 className="section-title">{editing ? 'Edit Door Design' : 'Add New Door Design'}</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                {[
                  { key: 'name',        label: 'Design Name',    ph: 'e.g. 2-Panel Carved Door', req: true },
                  { key: 'material',    label: 'Material / Wood', ph: 'e.g. Sheesham, Oak'              },
                  { key: 'price_from',  label: 'Starting Price (Rs.)', ph: '5000'                        },
                ...
                <div className="sm:col-span-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {form.image_url && (
                      <img src={form.image_url} alt="preview" className="w-16 h-16 object-cover rounded-xl border border-white/[0.1]" />
                    )}
          ...
          {loading ? (
            ...
            <div className="card p-16 text-center fade-in">
              <div className="w-14 h-14 bg-white/[0.03] rounded-2xl flex items-center justify-center mx-auto mb-4 border border-white/[0.05]">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-white/20"><rect x="3" y="1" width="18" height="22" rx="1"/></svg>
              </div>
              <p className="text-white/50 font-semibold mb-1">No door designs yet</p>
                ...
                <div key={item.id} className="card-hover overflow-hidden">
                  ...
                  <div className="relative h-44 bg-white/[0.03] overflow-hidden">
                    ...
                    <div className="absolute top-2 right-2">
                      <span className={`badge ${item.available !== false ? 'badge-green' : 'badge-red'}`}>
